...
export default function PromoBanner({ className = "" }: PromoBannerProps) {
  return (
    ...
    >
      <div className="absolute inset-y-0 left-1/3 w-1/2 bg-white/10 blur-3xl opacity-40" aria-hidden="true" />
      ...
      <div className="mx-auto flex max-w-screen-xl flex-col gap-4 px-4 text-white md:flex-row md:items-center md:justify-between md:px-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs uppercase tracking-[0.2em] text-white/80">Specjalna oferta</p>
            <h2 className="text-lg font-semibold md:text-2xl">Odbierz 25 darmowych lekcji z każdego tematu</h2>
          </div>
        </div>

        <div className="flex flex-col items-start gap-3 md:flex-row md:items-center">
          <p className="max-w-sm text-sm text-white/90 md:text-base">
            Dołącz do społeczności ProstoAngielski i ucz się swobodnie – dostęp bez zobowiązań, pełne 25 lekcji na start.
          </p>
          <Link
            href="/signup"
